...
export function LoginPage() {
  ...
  const connectWallet = async (walletId: string) => {
    ...
    if (result.success) {
      ...
      setStatus("success");
      setMessage(
        `钱包已验证：${result.address?.slice(0, 6)}...${result.address?.slice(-4)}。首次登录会自动创建账号。`
      );
      return;
    }

    if (address) {
      setMessage(`当前钱包：${address.slice(0, 6)}...${address.slice(-4)}。${result.error || "请重试登录。"}`);
    } else {
      setMessage(result.error || "钱包连接被取消，请重试。");
    }
        ...
        <div className="relative z-10 flex h-screen items-center justify-center px-10 py-12">
          <div className="w-full max-w-xl">
            <BrandLogo
              ...
              markClassName="grid h-9 w-9 place-items-center rounded-xl bg-white"
              textClassName="text-lg font-black tracking-wide xl:text-xl"
            />
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-8 sm:px-8 lg:h-screen lg:min-h-0">
        <div className="w-full max-w-md px-0 lg:px-6 lg:py-8">
          ...
          <div className="mb-6 text-center">
            <BrandLogo
              className="mx-auto mb-5 flex justify-center lg:hidden"
              iconClassName="h-7 w-7 text-hnb"
              markClassName="grid h-12 w-12 place-items-center rounded-2xl bg-ink text-white shadow-soft"
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            >
              {isConnecting || status === "loading" ? (
                ...
                <Image alt="" aria-hidden="true" className="h-5 w-5" height={20} src="/assets/icons/wallet.svg" width={20} />
              )}
              {isConnecting || status === "loading" ? "Connecting" : "Wallet"}
            </button>
          </div>
